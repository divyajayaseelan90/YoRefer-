...
{
    CGFloat togglekeyboardHeight, textFieldKeyboardHeight;
...
-(void)animateTextView:(UITextView*)textView isUp:(BOOL)isUp View:(UIView *)Myview postions:(CGFloat)postions
{
    ...
    NSArray *array = [[textView superview] subviews];
    if ([UIDevice currentDevice].userInterfaceIdiom==UIUserInterfaceIdiomPad)
    {
        if ([(UILabel *)[array objectAtIndex:2] tag] == 100000)
            
        {
            togglekeyboardHeight = 552.0;
        }
        else
            togglekeyboardHeight = 352.0;
    }
